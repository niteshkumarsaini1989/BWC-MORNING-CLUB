// Bharat Wellness Club - Browser-side Comprehensive Test Suite
(function runSuite() {
  const results = [];
  const errors = [];
  let passedCount = 0;
  ...
  // ----------------------------------------------------
  test('Auth Overlay & Direct Login as Master Admin', () => {
    quickLogin('ADMIN');
    ...
    assert(authOverlay.style.display === 'none', 'authOverlay should be hidden');
  });

  // ----------------------------------------------------
  // TEST 2: Demo Data Population & Verification
  // ----------------------------------------------------
  test('Demo Data Population & Dual-Mode State Engine', () => {
    loadDemoData(true);
    assert(Array.isArray(coaches) && coaches.length >= 4, `Expected at least 4 coaches, got ${coaches.length}`);
    assert(Array.isArray(consumers) && consumers.length >= 8, `Expected at least 8 consumers, got ${consumers.length}`);
    assert(Object.keys(attendanceData).length >= 10, `Expected at least 10 attendance records, got ${Object.keys(attendanceData).length}`);
    assert(Object.keys(btgData).length >= 8, `Expected at least 8 BTG diet records, got ${Object.keys(btgData).length}`);
    assert(Array.isArray(bmiRecords) && bmiRecords.length >= 8, `Expected at least 8 BMI records, got ${bmiRecords.length}`);
    assert(Object.keys(checkupData).length >= 8, `Expected at least 8 checkup records, got ${Object.keys(checkupData).length}`);
    assert(Array.isArray(salesRecords) && salesRecords.length >= 10, `Expected at least 10 sales records, got ${salesRecords.length}`);
  });

  // ----------------------------------------------------
  // TEST 3: Navigation & Tab Switching
  // ----------------------------------------------------
  test('Sidebar Navigation & Tab Views Activation', () => {
  ...
  // ----------------------------------------------------
  test('Role-Based Filtering (Admin vs Coach)', () => {
  ...
  // ----------------------------------------------------
  test('Consumer Master - Auto Code & WHO Ideal Weight Formula', () => {
  ...
  // ----------------------------------------------------
  // TEST 6: Attendance Register & Month View
  // ----------------------------------------------------
  test('Attendance Management (Daily Toggle & Month Register)', () => {
  ...
  // ----------------------------------------------------
  // TEST 7: BTG Food Diet Matrix & Modal
  // ----------------------------------------------------
  test('Food (BTG) Diet Tracking & Consumer Modal Tabs', () => {
    openSection('btg');
    renderBTG();
  ...
  // ----------------------------------------------------
  test('Product Tracking - Keyword Auto-complete & Multi-add', () => {
  ...
  // ----------------------------------------------------
  test('Reports Engine (Single Slips, Attendance, Diet, BMI, Full Combo)', () => {
    openSection('reports');
